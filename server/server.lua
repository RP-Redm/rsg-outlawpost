...
AddEventHandler('rsg-outlawpost:server:sellbloodmoney', function(amount)
	...
	if amountbloodmoney ~= nil then
		if amountbloodmoney >= amount then
			...
			TriggerClientEvent('QRCore:Notify', src, 'You sold ' ..amount.. ' blood money for $'..totalcash, 'success')
		else
			TriggerClientEvent('QRCore:Notify', src, 'You do not have enough blood money to do that!', 'error')
		end
	else
		TriggerClientEvent('QRCore:Notify', src, 'You do not have any blood money!', 'error')
	end
end)
...
AddEventHandler('rsg-outlawpost:server:sellgoldbars', function(amount)
	...
	if checkbars ~= nil then
		...
		if amountbars >= amount then
			...
			TriggerClientEvent('QRCore:Notify', src, 'You do not have enough gold bars to do that!', 'error')
		end
	else
		TriggerClientEvent('QRCore:Notify', src, 'You do not have any gold bars!', 'error')
	end
end)